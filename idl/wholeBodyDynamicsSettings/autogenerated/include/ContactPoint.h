...
#ifndef YARP_THRIFT_GENERATOR_STRUCT_ContactPoint
#define YARP_THRIFT_GENERATOR_STRUCT_ContactPoint
...
class ContactPoint : public yarp::os::idl::WirePortable {
public:
  // Fields
  double x;
  double y;
  ...
  ContactPoint(const double x,const double y,const double z) : x(x), y(y), z(z) {
  }

  // Copy constructor
  ContactPoint(const ContactPoint& __alt) : WirePortable(__alt)  {
    this->x = __alt.x;
    this->y = __alt.y;
    this->z = __alt.z;
  }
  ...
  // Assignment operator
  const ContactPoint& operator = (const ContactPoint& __alt) {
    this->x = __alt.x;
    this->y = __alt.y;
    this->z = __alt.z;
    return *this;
